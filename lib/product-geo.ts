...
const labels: Record<Lang, {
  ...
  publishedSpecsA: (model: string, facts: string) => string;
  ...
  en: {
    ...
    publishedSpecsA: (m, facts) => `The current ${m} record publishes these specifications: ${facts}. Refer to the specification table on this page for the complete model data.`,
  ...
  es: {
    ...
    whatA: (m, s) => `${m} es una cafetera portátil TK Classic. ${s}`, whoA: (m, u) => `${m} se presenta para estos escenarios de compra o uso publicados: ${u}. La configuración final debe confirmarse según el mercado y el pedido.`, specA: (m, p, b, c) => `La ficha actual de ${m} indica presión de ${p}, batería ${b} y capacidad o formato de taza ${c}. Consulte la tabla de esta página para ver todos los datos publicados.`, publishedSpecsA: (m, facts) => `La ficha actual de ${m} publica estas especificaciones: ${facts}. Consulte la tabla de esta página para ver todos los datos del modelo.`, formatA: (m, a) => `La ficha actual de ${m} indica: ${a}. Confirme los adaptadores y accesorios incluidos en la cotización formal.`, quoteA: (m) => `Envíe la cantidad, destino, marca, embalaje y accesorios mediante el formulario. TK Classic confirmará la configuración disponible de ${m}, las condiciones comerciales y los documentos aplicables en la cotización.`,
  },
  pt: {
    ...
    whatA: (m, s) => `${m} é uma cafeteira portátil TK Classic. ${s}`, whoA: (m, u) => `${m} é posicionada para estes cenários publicados de compra ou uso: ${u}. A configuração final deve ser confirmada para o mercado e o pedido.`, specA: (m, p, b, c) => `O registro atual da ${m} informa pressão de ${p}, bateria ${b} e capacidade ou formato de copo ${c}. Consulte a tabela desta página para todos os dados publicados.`, publishedSpecsA: (m, facts) => `O registro atual da ${m} publica estas especificações: ${facts}. Consulte a tabela desta página para todos os dados do modelo.`, formatA: (m, a) => `O registro atual da ${m} informa: ${a}. Confirme os adaptadores e acessórios incluídos na cotação formal.`, quoteA: (m) => `Envie quantidade, destino, marca, embalagem e acessórios pelo formulário. A TK Classic confirmará a configuração disponível da ${m}, as condições comerciais e os documentos aplicáveis na cotação.`,
  },
  fr: {
  ...
  ar: {
    ...
    whatA: (m, s) => `${m} ماكينة قهوة محمولة من TK Classic. ${s}`, whoA: (m, u) => `يُعرض طراز ${m} لسيناريوهات الشراء أو الاستخدام المنشورة التالية: ${u}. يجب تأكيد التكوين النهائي حسب السوق والطلب.`, specA: (m, p, b, c) => `تذكر بيانات ${m} الحالية ضغط ${p} وبطارية ${b} وسعة أو صيغة كوب ${c}. راجع جدول المواصفات في هذه الصفحة لجميع البيانات المنشورة.`, publishedSpecsA: (m, facts) => `تنشر بيانات ${m} الحالية المواصفات التالية: ${facts}. راجع جدول المواصفات في هذه الصفحة لجميع بيانات الطراز.`, formatA: (m, a) => `تذكر بيانات ${m} الحالية: ${a}. يجب تأكيد المحولات والملحقات المطلوبة في عرض السعر الرسمي.`, quoteA: (m) => `أرسل الكمية والسوق والهوية والتغليف والملحقات عبر نموذج الاستفسار. ستؤكد TK Classic تكوين ${m} المتاح والشروط التجارية والوثائق المنطبقة في عرض السعر.`,
  },
  zh: {
    ...
    whatA: (m, s) => `${m} 是 TK Classic 的便携式咖啡机。${s}`, whoA: (m, u) => `${m} 已公布的采购或使用场景包括：${u}。最终配置需要根据目标市场和订单要求确认。`, specA: (m, p, b, c) => `${m} 当前资料列出的压力为 ${p}，电池信息为 ${b}，杯容量或杯型为 ${c}。完整的已公布型号数据请查看本页参数表。`, publishedSpecsA: (m, facts) => `${m} 当前公开的参数包括：${facts}。该型号全部已发布资料请查看本页参数表。`, formatA: (m, a) => `${m} 当前资料列出：${a}。所需转接器和包装内配件应在正式报价中确认。`, quoteA: (m) => `请通过询盘表提交预计数量、目的地、品牌、包装和配件需求。TK Classic 会在报价中确认 ${m} 可提供的配置、商务条款和适用资料。`,
  },
  ru: {
    ...
    whatA: (m, s) => `${m} — портативная кофемашина TK Classic. ${s}`, whoA: (m, u) => `${m} позиционируется для следующих опубликованных сценариев закупки или использования: ${u}. Итоговую конфигурацию следует подтвердить для выбранного рынка и заказа.`, specA: (m, p, b, c) => `В текущих данных ${m} указаны давление ${p}, батарея ${b} и ёмкость или формат чашки ${c}. Полные опубликованные данные приведены в таблице на этой странице.`, publishedSpecsA: (m, facts) => `В текущей карточке ${m} опубликованы характеристики: ${facts}. Полные данные модели приведены в таблице на этой странице.`, formatA: (m, a) => `В текущих данных ${m} указано: ${a}. Требуемые адаптеры и комплект поставки подтверждаются в официальном предложении.`, quoteA: (m) => `Отправьте через форму количество, рынок назначения, требования к брендингу, упаковке и аксессуарам. TK Classic подтвердит доступную конфигурацию ${m}, коммерческие условия и применимые документы в предложении.`,
  },
};

export function getProductGeo(product: Product, lang: Lang) {
  const verifiedRecord = productGeoRecords[product.model];
