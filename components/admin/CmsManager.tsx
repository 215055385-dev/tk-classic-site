...
function MediaGrid({ items, onEdit, onDelete }: { items: Item[]; onEdit: (x: Item) => void; onDelete: (x: Item) => void }) {
  return <div className="cms-media-grid">{items.map((item, index) => { const issues = contentReadinessIssues("media", item); return <article key={item.id} className="cms-media-card"><div className="cms-media-preview">{text(item.type) === "VIDEO" ? <video src={text(item.publicUrl)} muted preload="metadata"/> : item.publicUrl ? <Image unoptimized loading={index === 0 ? "eager" : "lazy"} src={text(item.publicUrl)} alt={text(item.altText)} width={480} height={360}/> : <ImageIcon/>}</div><div><strong>{text(item.originalName)}</strong><small>{text(item.category)} · {Math.max(0, Number(item.bytes) / 1024 / 1024).toFixed(2)} MB</small><span className={Number(item.referenceCount) ? "cms-reference is-used" : "cms-reference"}>{Number(item.referenceCount) ? `已被 ${Number(item.referenceCount)} 处内容使用` : "当前未被引用"}</span><span className={`cms-readiness-chip ${issues.length ? "is-incomplete" : "is-ready"}`}>{issues.length ? "缺少图片说明" : "说明完整"}</span></div><div className="cms-actions"><a href={text(item.publicUrl)} target="_blank" rel="noreferrer" aria-label="查看"><ExternalLink size={16}/></a><button onClick={() => onEdit(item)} aria-label="编辑"><Edit3 size={16}/></button><button className="is-danger" onClick={() => void onDelete(item)} aria-label="删除"><Trash2 size={16}/></button></div></article>; })}</div>;
}
