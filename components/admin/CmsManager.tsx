...
const labels = {
  products: { eyebrow: "产品 CMS", title: "产品管理", description: "管理真实产品资料、发布状态、参数、卖点、主图与 SEO。官方型号已锁定。", add: "新增产品" },
...
function Editor({ resource, item, media, setItem, close, save, saving }: { resource: Resource; item: Item; media: MediaOption[]; setItem: (x: Item) => void; close: () => void; save: (e: React.FormEvent<HTMLFormElement>) => void; saving: boolean }) {
  ...
  return <div className="cms-modal-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget) close(); }}><form className="cms-modal" onSubmit={save}><header><div><span>{item.id ? "编辑" : "新增"}</span><h2>{labels[resource].title}</h2></div><button type="button" onClick={close} aria-label="关闭"><X/></button></header><div className="cms-form-grid">
    {resource === "products" ? <>
      <Field label="产品型号"><input value={text(item.model)} disabled={Boolean(item.lockedModel)} onChange={(e) => change("model", e.target.value)} required /></Field><Field label="URL 路径"><input value={text(item.slug)} onChange={(e) => change("slug", e.target.value)} required /></Field>
      ...
      <Field label="官方主图"><select value={text(item.heroMediaId)} onChange={(e) => change("heroMediaId", e.target.value || null)}><option value="">暂不关联</option>{media.map((x) => <option key={x.id} value={x.id}>{x.originalName}</option>)}</select></Field>
  ...
  </div><footer><button type="button" className="cms-secondary" onClick={close}>取消</button><button className="cms-primary" disabled={saving}>{saving ? "保存中…" : "保存并同步"}</button></footer></form></div>;
}
