...
type Inquiry = {
  ...
  salesDeliveryStatus: string; customerDeliveryStatus: string;
  salesDeliveredAt: string; customerDeliveredAt: string;
  deletedAt: string;
  assignedTo: "Bowie" | "Leo" | "";
};
type EmailHealth = { ready: boolean; apiKeyConfigured: boolean; senderConfigured: boolean; senderAddress: string; siteDomain: string; domainStatus: string; recipientCount: number; webhookConfigured: boolean; checkedAt: string; error: string };
...
const deliveryLabels: Record<string, string> = {
  unknown: "未记录", sent: "已接受", delivered: "已送达", delayed: "延迟中",
  bounced: "已退信", complained: "被投诉", failed: "发送失败", suppressed: "已拦截",
};
function deliveryLabel(status: string, sent: boolean | null) {
  return deliveryLabels[status] ?? (sent === null ? "未记录" : sent ? "已接受" : "发送失败");
}
function deliveryClass(status: string, sent: boolean | null) {
  if (status === "delivered") return "is-sent";
  if (["bounced", "complained", "failed", "suppressed"].includes(status) || sent === false) return "is-failed";
  return "is-muted";
}
...
export function InquiryWorkspace() {
  ...
  return <section className="inquiry-workspace">
    ...
    {emailHealth ? <div className={`inquiry-email-health ${emailHealth.ready ? "is-ready" : "is-warning"}`} role="status">{emailHealth.ready ? <CheckCircle2 size={21}/> : <CircleAlert size={21}/>}<div><strong>{emailHealth.ready ? "邮件服务运行正常" : "邮件服务需要检查"}</strong><p>{emailHealth.ready ? `${emailHealth.siteDomain} 已验证，真实送达、退信与投诉追踪已启用。` : emailHealth.error || (!emailHealth.webhookConfigured ? "邮件 Webhook 尚未配置，无法追踪真实送达结果。" : `域名状态：${emailHealth.domainStatus}；请检查发件地址和域名验证。`)}</p><span>发件：{emailHealth.senderAddress || "未配置自定义地址"} · 销售收件人：{emailHealth.recipientCount} 个 · 检查时间：{formatDate(emailHealth.checkedAt)}</span></div></div> : null}
