...
type NavKey = "home" | "products" | "accessories" | "oem" | "factory" | "resources" | "contact";
...
export const resourcesCopy: Record<Lang, { label: string; overview: [string, string]; guides: [string, string]; scenarios: [string, string]; wholesale: [string, string] }> = {
  en: { label: "Resources", overview: ["Resources hub", "All buyer tools in one place"], guides: ["Buyer guides", "Practical sourcing answers"], scenarios: ["Use scenarios", "Camping, travel and car coffee"], wholesale: ["USA wholesale", "DQ-001 and DQ-010 sourcing"] },
  es: { label: "Recursos", overview: ["Centro de recursos", "Todas las herramientas de compra"], guides: ["Guías de compra", "Respuestas prácticas de sourcing"], scenarios: ["Escenarios de uso", "Camping, viaje y automóvil"], wholesale: ["Mayoristas en EE. UU.", "Sourcing de DQ-001 y DQ-010"] },
  pt: { label: "Recursos", overview: ["Central de recursos", "Todas as ferramentas de compra"], guides: ["Guias de compra", "Respostas práticas de sourcing"], scenarios: ["Cenários de uso", "Camping, viagem e carro"], wholesale: ["Atacado nos EUA", "Sourcing de DQ-001 e DQ-010"] },
  fr: { label: "Ressources", overview: ["Centre de ressources", "Tous les outils d’achat"], guides: ["Guides d’achat", "Réponses pratiques de sourcing"], scenarios: ["Scénarios d’usage", "Camping, voyage et voiture"], wholesale: ["Grossistes aux États-Unis", "Sourcing DQ-001 et DQ-010"] },
  ar: { label: "الموارد", overview: ["مركز الموارد", "جميع أدوات الشراء في مكان واحد"], guides: ["أدلة الشراء", "إجابات عملية للمشتريات"], scenarios: ["سيناريوهات الاستخدام", "التخييم والسفر والسيارة"], wholesale: ["البيع بالجملة في الولايات المتحدة", "توريد DQ-001 وDQ-010"] },
  zh: { label: "资源", overview: ["资源中心", "集中查看全部采购工具"], guides: ["采购指南", "真实资料整理的采购答案"], scenarios: ["使用场景", "露营、旅行与车载咖啡"], wholesale: ["美国批发", "DQ-001 与 DQ-010 采购"] },
  ru: { label: "Ресурсы", overview: ["Центр ресурсов", "Все инструменты закупщика"], guides: ["Руководства покупателя", "Практические ответы по закупкам"], scenarios: ["Сценарии использования", "Кемпинг, путешествия и автомобиль"], wholesale: ["Оптовые поставки в США", "Закупка DQ-001 и DQ-010"] },
};
  ...
  return (
    ...
    <nav className="primary-nav" aria-label={ariaLabel}>
      ...
      <div className="resources-nav-group">
        <details className="resources-nav-menu" suppressHydrationWarning>
          <summary aria-label={`${resources.label} menu`} aria-current={current === "resources" ? "page" : undefined}>
            <span>{resources.label}</span><ChevronDown size={14} aria-hidden="true" />
          </summary>
